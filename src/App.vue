...
function showSdkError(data: any) {
  ...
  if (msg) { console.error('[SDK Error]', data); sdkErrorMsg.value = msg }
}
...
const showCredModal = ref(showCredOnStart)
...
// SDK 未配置时自动弹出凭证弹窗
if (!sdkConfigured.value) showCredModal.value = true
...
function handleUserInput(text: string, source: InputSource, widgetIntent?: string) {
  ...
  // 敏感信息（卡号/身份证/原因）不显示明文，LLM 对话模式正常显示
  if (def.inputMode === 'llm') {
...
function handleLLMIntent(parsed: { reply: string; intent: string }) {
  try {
    ...
    console.error('[handleLLMIntent]', e)
...
watch(currentState, (newState) => {
  ...
  if (def.subtitleText) {
    // 先立即显示新字幕，避免旧字幕残留；SDK subtitle_on 事后会再次更新
    sdkStore.subtitle.value = def.subtitleText
    try { speakText(def.subtitleText) } catch {}
  }
})
...
<template>
  <div class="app-root">
    ...
    <div v-if="isRecording && asrText" class="asr-text-overlay">{{ asrText }}</div>
    ...
    <div class="main-area">
      ...
      <SubtitleOverlay :is-pip="state.layout === 'pip'" />
    </div>
