...
function getProgressSummary(state: StateName, session: SessionData): string {
  ...
  if (session.receiptNo) lines.push(`- 受理单号：${escapeForPrompt(session.receiptNo)}`)
...
function getCurrentTask(state: StateName): string {
  switch (state) {
    ...
    case 'done':
      return '告知用户挂失已受理，给出受理单号。⚠️ 你已经完成了挂失流程，用户只有三条路：(1)说"挂失""重新办理""再来一次""办其他业务""返回首页"→返回 welcome；(2)说"修改信息""我要修改"→返回 modify_field；(3)说其他无关的话，intent 返回 stay，友好告知"挂失已受理完成，请问还需要什么帮助？"。绝对不要返回 ask_card_no / ask_id_no / ask_reason。'
    case 'unsupported':
