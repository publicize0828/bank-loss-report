...
const props = defineProps<{ disabled: boolean; recording: boolean; asrDisabled: boolean; maxlength?: number }>()
const emit = defineEmits<{ voice: [text: string]; text: [text: string]; 'ptt-start': []; 'ptt-end': []; input: [text: string] }>()
const textInput = ref('')

const hasLimit = computed(() => props.maxlength != null && props.maxlength < 524288)
const charCount = computed(() => textInput.value.length)
const limitNear = computed(() => hasLimit.value && charCount.value >= props.maxlength! * 0.85)
const limitReached = computed(() => hasLimit.value && charCount.value >= props.maxlength!)
...
<template>
  <div class="control-bar">
    ...
    <div class="text-input-wrap">
      <input :value="textInput" @input="onInput" type="text" placeholder="输入文字..." :disabled="disabled" :maxlength="maxlength ?? 524288" @keyup="onKeyup" />
      <span v-if="hasLimit" class="char-count" :class="{ near: limitNear, reached: limitReached }">{{ charCount }}/{{ maxlength }}</span>
...
.char-count { font-size: 11px; color: #bbb; flex-shrink: 0; white-space: nowrap; }
.char-count.near { color: #f59e0b; }
.char-count.reached { color: #ef4444; font-weight: 600; }
</style>
